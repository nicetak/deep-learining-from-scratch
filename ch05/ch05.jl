...
# ╔═╡ a7f1c57b-b594-4e44-830e-a62bf988af5b
begin
	using PlutoUI
	using LinearAlgebra
	using Statistics
...
# ╔═╡ 98d2f3f4-7c32-11ec-2c51-e5c234aea6a5
md"""
# Chapter 5
#### Nicetak
...
# ╔═╡ 1b4cedeb-7246-4163-8ecd-e9484ce71704
md"""
### Multiplication Layer
"""

# ╔═╡ f6269604-e552-4877-80b1-751091265e7b
mutable struct MulLayer{T}
	x::T
	y::T
end

# ╔═╡ 3d93fafb-4c26-4fab-aa6f-297b01304ff1
forward(self::MulLayer) = self.x * self.y

# ╔═╡ 0a72a1f5-6ffc-4163-b261-e715b837c3cd
backward(self::MulLayer, dz) = (dz * self.y, dz * self.x)

# ╔═╡ 71f35050-44b7-4ec1-bec0-71e16900bd6c
md"""
### Addition Layer
"""

# ╔═╡ 8cbe689e-9b25-4bf4-98ac-79dc53cd708d
mutable struct AddLayer{T}
	x::T
	y::T
end

# ╔═╡ 2467a527-99ad-4447-9d45-7b25172fe854
forward(self::AddLayer) = self.x + self.y

# ╔═╡ 9b0829f9-db35-4cab-9821-0f7a38b713f2
backward(self::AddLayer, dz) = dz, dz

# ╔═╡ 1e76f033-3199-4aae-8899-1a7f8c4fb7fd
md"""
### ReLU Layer
"""

# ╔═╡ 086fa6b2-10e8-4fc2-9c8c-00218b3da8b2
mutable struct ReluLayer
	mask::Bool
end

# ╔═╡ 0296dfc7-9382-40d6-921a-73b63890dd72
function forward(self::ReluLayer, x)
	self.mask = x ≤ zero(x)
	return self.mask ? zero(x) : x 
end

# ╔═╡ a5a3ce7e-e6a9-4600-a5af-f1f1cdfa3a70
backward(self::ReluLayer, dz) = self.mask ? zero(dz) : dz

# ╔═╡ 1ca44b1e-7f59-494f-9a3c-776e243dc921
md"""
### Sigmoid Layer
"""

# ╔═╡ 2d45ed9a-6ab6-43f3-8fdb-887f4773bf86
mutable struct Sigmoid{T}
	out::T
end

# ╔═╡ 404e5a4c-01bd-4b19-b8ef-c29141ee2986
...
# ╔═╡ 21fbe44f-35fe-49df-977b-21a87827e0d5
backward(self::Sigmoid, dz) = dz * (1 - self.out) * self.out

# ╔═╡ 08cba512-5542-4197-ac12-8057042be45b
let
	apple = 100.0
	apple_num = 2.0
	tax = 1.1
	...
	apple_price = forward(mul_apple_layer)
	
	mul_tax_layer = MulLayer(apple_price, tax)
	price = forward(mul_tax_layer)
	...
	Print("$(round(price))\n$dapple, $(round(dapple_num)), $dtax")
end

# ╔═╡ 8c96c2b1-1c9d-455c-8938-55d2e29b8987
let
	apple = 100.0
	apple_num = 2.0
	orange = 150.0
	...
	orange_price = forward(mul_orange_layer)

	add_apple_orange_layer = AddLayer(apple_price, orange_price)
	all_price= forward(add_apple_orange_layer)
	...
	@show dorange, dorange_num = backward(mul_orange_layer, dorange_price)
	dapple, dapple_num = backward(mul_apple_layer, dapple_price)

	Print("$(round(price))\n$(round.([dapple_num, dapple, dorange, dorange_num, dtax], digits = 1))")
	
end

# ╔═╡ d18fa85d-58e9-4fb9-9f12-61a5ba56fee3
md"""
### Affine Layer
"""

# ╔═╡ e6ab3d06-b4f3-423a-b77e-493b8fa80cc9


# ╔═╡ 5ffd566e-f567-4d96-88bd-43ca7e3e0f18
