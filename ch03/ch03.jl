...
# ╔═╡ 704d2a72-279a-4335-a91b-7d884f900239
begin
	using PlutoUI
	using LinearAlgebra
	using Statistics
...
# ╔═╡ 98b1d2a6-795c-11ec-1ab7-6d00d30d5ee4
md"""
# Chapter 3
#### Nicetak
...
# ╔═╡ 4a6292c1-8756-47be-9761-2ccafc33a3c9
begin
	sigmoid(x) = 1 / (1 + exp(-x))
	identity(x) = x
end

# ╔═╡ 7f9103c5-24dd-4692-9ea2-d2e0ca4f8daa
function softmax(xs)
	x̄ = maximum(xs)
	exp_xs = exp.(xs .- x̄)
...
# ╔═╡ 09f8d162-b9e0-4cc7-9742-67c7d03f9125
with_terminal() do
	X = [1.0, 0.5]
	W₁ = [0.1 0.3 0.5
		  0.2 0.4 0.6]
...
# ╔═╡ c6f4084b-4635-4c19-a690-033ef18ec13b
md"""
## MNIST
"""

# ╔═╡ 4659723b-a8fd-4f32-94f4-0bce83cb5dd5
begin
	# Load dataset
...
# ╔═╡ 88d76f1b-85a4-4f52-91d5-f5510c427a90
struct Network{T}
	W₁::Matrix{T}
	W₂::Matrix{T}
	W₃::Matrix{T}
...
# ╔═╡ fae09695-321d-4dba-b043-c85360f8e2a8
function predict(network::Network, x)
	@unpack W₁, W₂, W₃, b₁, b₂, b₃ = network
	...
	return argmax(y) - 1 # Because from 0-9
end

# ╔═╡ dd400521-ef62-4048-8552-e85e249d027c
begin
	file = h5open("sample_weight.hdf5", "r")
	network = Network(
		[copy(read(file, k)') for k ∈ ("W1", "W2", "W3")]...,
		[read(file, k) for k ∈ ("b1", "b2", "b3")]...
	)

	# Accuracy
	acc = mean(y_test[i] == predict(network, x_test[:, i]) for i = 1:length(y_test))

	Print("Accuracy: $(round(acc, digits = 4))")
end
