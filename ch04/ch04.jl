...
# ╔═╡ 6daeac8c-1eb8-428d-8bcf-f958d5996f54
begin
	using PlutoUI
	using LinearAlgebra
	using Statistics
	...
	using Flux: onehotbatch, gradient, params

	using Plots
end

# ╔═╡ b610e2e4-7a13-11ec-290f-138b69bf298c
md"""
# Chapter 4
#### Nicetak
...
# ╔═╡ 84a90be3-0c11-4c19-8f4a-4799d7a787f9
function softmax(xs)
	x̄ = maximum(xs)
	exp_xs = exp.(xs .- x̄)
...
# ╔═╡ 3aa17eca-6adc-4090-9bd5-bda09651c5cd
sigmoid(x) = 1 / (1 + exp(-x))

# ╔═╡ 8cc0d07a-a7fd-42f5-a970-cfa1dbacda53
cross_entropy_error(𝐲̂, 𝐲; δ = 1e-7) = -mean(y * log(ŷ + δ) for (ŷ, y) ∈ zip(𝐲̂, 𝐲))

# ╔═╡ 880458bd-9ddb-41e3-94a5-12ec70ef39b3
numerical_gradient(f, x; ϵ = 1e-4) = begin
...
# ╔═╡ 7245cde3-11dc-4c88-8b0d-14f29ff7c88e
md"""
## Single Layer
"""

# ╔═╡ d73fb049-6b00-4269-8a8c-5e753a5ba6d5
begin
	@with_kw mutable struct SimpleNet{T}
		W::Matrix{T} = randn(2, 3)
	end

	predict(𝐱, net::SimpleNet,) = softmax(net.W'𝐱)
	loss(𝐱, 𝐲, net::SimpleNet) = cross_entropy_error(predict(𝐱, net), 𝐲)
	
end

# ╔═╡ 8e034a4e-6ecc-44a4-9c5d-0fdc580d87cf
md"""
## Dual Layers
"""

# ╔═╡ e9f02949-2933-4f17-b900-f7ac10bd1602
begin
	@with_kw mutable struct TwoLayerNet{T}
		params::Dict{T}
		grads::Dict{T}
	end

	TwoLayerNet(size_input, size_hidden, size_output; std_weight_init=0.01) = begin
		
		params = Dict(
			"W₁" => std_weight_init * randn(size_input, size_hidden),
			"W₂" => std_weight_init * randn(size_hidden, size_output),
			"b₁" => zeros(size_hidden),
			"b₂" => zeros(size_output)
		)
		
		grads = Dict(
			"W₁" => zeros(size(params["W₁"])),
			"W₂" => zeros(size(params["W₂"])),
			"b₁" => zeros(size(params["b₁"])),
			"b₂" => zeros(size(params["b₂"]))
		)

		TwoLayerNet(params, grads)
	end

	predict(x, net::TwoLayerNet) = begin
		W₁, W₂ = net.params["W₁"], net.params["W₂"]
		b₁, b₂ = net.params["b₁"], net.params["b₂"]
	...
	loss(𝐱, 𝐲, net::TwoLayerNet) = cross_entropy_error(predict(𝐱, net), 𝐲)
	accuracy(𝐱, 𝐲, net::TwoLayerNet) = 
		mean(ŷ == y for (ŷ, y) ∈ zip(predict(𝐱, net), 𝐲))

	
	function numerical_gradient!(𝐱, 𝐲, net::TwoLayerNet)
		for key ∈ ("W₁", "W₂", "b₁", "b₂")
			f(W) = begin
				net.params[key] = W
				loss(𝐱, 𝐲, net)
			end
			net.grads[key] = numerical_gradient(f, net.params[key])
		end
	end
	
end

# ╔═╡ 267c11f3-e25a-40b0-b6b3-cfe381bd3b82
with_terminal() do
	net = SimpleNet()
	𝐱 = [0.6, 0.9]
	@show 𝐲̂ = predict(𝐱, net)
	𝐲 = [0.0, 0.0, 1.0]
	...
	@show numerical_gradient(f, net.W)
end

# ╔═╡ 8b431ee9-9e9d-43c3-8ed0-443ca7d64ef4
md"""
### MNIST
"""

# ╔═╡ 64f3bdfc-f942-4da3-93d4-96fafb939c9a
begin
...
# ╔═╡ 3037613e-b1ea-4976-9f45-0e0aeaa249fd
# I don't think it will end in a reasonable time
function main()
	# Parameters
	n_iter = 10^4
	size_train = size(x_train)[2]
	size_batch = 100
	iter_per_epoch = size_train // size_batch
	η = 0.1 # Learning Rate

	# Learning
	accs_train = Vector{Float64}
	accs_test = Vector{Float64}
	
	net = TwoLayerNet(784, 100, 10)
	for i = 1:n_iter
		ids = sample(1:size_train, size_batch)
		x_batch = x_train[:, ids]
		y_batch = y_train[:, ids]

		# Update Params
		grads = numerical_gradient!(x_batch, y_batch, net)
		for key ∈ ("W₁", "W₂", "b₁", "b₂")
			net.params[key] -= η * net.grads[key]
		end

		if i % iter_per_epoch == 0
			acc_train = accuracy(x_train, y_train, net)
			acc_test = accuracy(x_test, y_test, net)
			push!(accs_train, acc_train)
			push!(accs_test, acc_test)
			println("acc. train: $acc_train")
			println("acc. test: $acc_test")
		end
	end
